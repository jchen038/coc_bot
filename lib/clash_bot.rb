...
class ClashBot
	...
	def commands
		["our_clan", "search (clan name)", "clan_info (clan tag)", "war_log (clan_tag)", "search_cocp (clan tag)"]
	end
	...
	def search_clan(name)
		clans = @api.search_clans(name: name, min_members: 10).items
		clans[0..20].map{|clan| self.class.display_clan(clan)}.join("\n")
	end

	def clan_info(tag)
		clan = @api.clan_info(tag)
		self.class.display_clan(clan)
	end

	def war_log(tag)
		logs = @api.clan_war_log(tag)
		logs.items[0..20].map{|log| self.class.display_log(log)}.join("\n")
	end
	...
	def self.search_cocp(tag)
		url = "https://cocp.it/clan/"
		page = Nokogiri::HTML(open("#{url}#{tag}"))
		page.css("div.rocknrollanimal").map do |stat|
			display_cocp(stat)
		end.join("\n")
	end
	...
	def self.display_cocp(war_result)
